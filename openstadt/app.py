"""OpenStadt application factory."""

import inspect

import click
from flask import Flask, render_template
from flask_dance.consumer.storage.sqla import SQLAlchemyStorage
from flask_dance.contrib.github import make_github_blueprint
from flask_dance.contrib.google import make_google_blueprint
from flask_security import Security, SQLAlchemyUserDatastore, current_user

import openstadt.commands as commands
from openstadt.api.routes import api
from openstadt.extensions import babel, cache, db, debug_toolbar, mail, session
from openstadt.portal.views import portal
from openstadt.public.views import public
from openstadt.settings import Config
from openstadt.user.forms import ExtendedRegisterForm, OAuthAwareChangePasswordForm
from openstadt.user.models import OAuth, Role, User, WebAuthn
from openstadt.user.views import bp_user


def create_app(config_object=Config):
    app = Flask(__name__)
    app.config.from_object(config_object)

    register_blueprints(app)
    register_extensions(app)
    register_errorhandlers(app)
    register_shellcontext(app)
    register_commands(app, commands)
    return app


def locale_selector():
    return "de"  # Default to German for civic platform


def register_extensions(app):
    cache.init_app(app)
    db.init_app(app)
    user_datastore = SQLAlchemyUserDatastore(db, User, Role, webauthn_model=WebAuthn)
    Security(
        app,
        user_datastore,
        register_form=ExtendedRegisterForm,
        change_password_form=OAuthAwareChangePasswordForm,
    )
    mail.init_app(app)
    debug_toolbar.init_app(app)

    if app.config.get("SESSION_TYPE") == "sqlalchemy":
        app.config["SESSION_SQLALCHEMY"] = db
    session.init_app(app)

    babel.init_app(
        app,
        locale_selector=locale_selector,
        default_domain="messages",
        default_locale="de",
    )


def register_blueprints(app):
    app.register_blueprint(api)
    app.register_blueprint(bp_user)
    app.register_blueprint(public)
    app.register_blueprint(portal)

    # OAuth setup
    if app.config.get("GOOGLE_AUTH_ENABLED") and app.config.get(
        "GOOGLE_OAUTH_CLIENT_ID"
    ):
        google_bp = make_google_blueprint(
            client_id=app.config.get("GOOGLE_OAUTH_CLIENT_ID"),
            client_secret=app.config.get("GOOGLE_OAUTH_CLIENT_SECRET"),
            scope=[
                "https://www.googleapis.com/auth/userinfo.profile",
                "https://www.googleapis.com/auth/userinfo.email",
                "openid",
            ],
            reprompt_select_account=False,
        )
        google_bp.storage = SQLAlchemyStorage(OAuth, db.session, user=current_user)
        app.register_blueprint(google_bp, url_prefix="/login")

    if app.config.get("GITHUB_AUTH_ENABLED") and app.config.get(
        "GITHUB_OAUTH_CLIENT_ID"
    ):
        github_bp = make_github_blueprint(
            client_id=app.config.get("GITHUB_OAUTH_CLIENT_ID"),
            client_secret=app.config.get("GITHUB_OAUTH_CLIENT_SECRET"),
            scope=["user:email"],
        )
        github_bp.storage = SQLAlchemyStorage(OAuth, db.session, user=current_user)
        app.register_blueprint(github_bp, url_prefix="/login")


def register_errorhandlers(app):
    def render_error(error):
        error_code = getattr(error, "code", 500)
        return render_template(f"errors/{error_code}.html"), error_code

    for errcode in [401, 404, 500]:
        app.errorhandler(errcode)(render_error)


def register_shellcontext(app):
    def shell_context():
        from openstadt.api.models import City, District, Layer, POI

        return {
            "db": db,
            "User": User,
            "Role": Role,
            "City": City,
            "Layer": Layer,
            "POI": POI,
            "District": District,
        }

    app.shell_context_processor(shell_context)


def register_commands(app: Flask, commands_module):
    for _name, obj in inspect.getmembers(commands_module):
        if isinstance(obj, click.Command | click.Group):
            app.cli.add_command(obj)
